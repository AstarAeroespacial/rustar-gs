...
async fn listen_mqtt(mut receiver: MqttReceiver){
    receiver.subscribe("hello-mqtt").await.expect("Error connecting to mqtt. Is the broker on?");
    for _ in 0..7 {
...
async fn send_messages(sender: MqttSender){
    for i in 0..7{
        ...
        let _ = sender.publish("hello-mqtt", format!("Hello! This is message n{}", i+1).as_str()).await.expect("Error sending message. Is the broker on?");
    }
}
