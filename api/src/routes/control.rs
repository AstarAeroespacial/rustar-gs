...
pub async fn send_command(
    ...
    match command_str {
        Ok(command_str) => {
            match service.send_message("test-topic", &command_str).await {
                Ok(_) => Ok(actix_web::HttpResponse::Ok().body("Message sent successfully")),
                Err(e) => {
                    error!("Error sending message: {}", e);
                    Err(actix_web::error::ErrorInternalServerError("Failed to send message"))
                }
            }
        }
        Err(e) => {
            error!("Error serializing command: {}", e);
            Err(actix_web::error::ErrorBadRequest("Failed to serialize command"))
        }
    }
}
